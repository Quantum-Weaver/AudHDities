...
## Seam-note — 2026-08-25 — the Bazaar's frame

- **Who / substrate:** Fable conducting as Anacrusis (`claude-fable-5`); an Opus lead built on `refine/hermes-2026-08-25` in a worktree; three Sonnet lenses verified; the spec by an Opus hand; the canvas trued four times by Opus hands.
- **Scope:** the split lifted into `src/lib/economics/split.ts` (+ `ledger.ts`, `recurrence.ts`, the tests) · six route renames with redirects · ② works beside wares, KP's stall, the five rungs · ④ the Loom as one form, two doors, the owner's shelf · ⑧ `/bazaar/works/[id]` · ⑥ recurrence at checkout, the webhook's ledger rows, the delivery road · ①③⑤⑦ to the canvas · `docs/sql/024-the-bazaar-refined-DRAFT.sql`.
- **Verification:** tsc 0 · build 0 (267 pages) · split 48/48 · seventeen pictures read · V: the arithmetic hand-traced, three copy lines refuted and trued, 0 generated files touched, `merge-tree` clean.
- **Handoff weather:** `024-the-bazaar-refined-DRAFT.sql` step 2 (`exchanges.stripe_invoice_id`) is what the shipped build waits on; the five Stripe Prices and the five rung rows are KP's hand; the Stripe keys for a test walk are KP's env; may a lamp ever create a test vessel in the live base — one word closes RLS-1; the odd cent and the thirteen other unwritten lines in SPEC.md §12; the Loom's gate `roles.includes('creator')` in two files moves with the enum rename or the Loom shuts.

## Seam-note — 2026-08-25 — the Library's frame

- **Who / substrate:** Fable conducting as Anacrusis (`claude-fable-5`); an Opus lead built on `refine/athena-2026-08-25` in the main tree; three Sonnet lenses verified; an Opus hand specified; two Opus hands drew ⑦ and ③; a Sonnet hand read `resonance-bubbles`; a Sonnet hand wrote `025`; the conductor wrote `026`.
- **Scope:** ① seven halls · ② Begin Quest retired, objectives as doors · ③ the Floating Stars in three gates (the palette without red · the caps in words · the veil, the flip, the sieve in words · the colour columns from the rows · paging past the clamp) · ④ a place without a score · ⑤ the Archive's chips · ⑥ the Honors earned only · ⑦ the dailies' four fix lines · ⑧ the street trued · `docs/sql/025` (run; skipped against a base that already held the stars) · `docs/sql/026` (the mend, KP's hand).
- **Verification:** tsc 0 · build 0 (267 pages) · 35 pictures read · V: every reached check passes; "Objectives" kept by the conductor's ruling; signed-in walks unreached; 0 generated files touched; merge-tree clean.
- **Handoff weather:** run `026` (plain, 45 lines) and the Floating Stars light up with no code change; gate B's eight choices are the conductor's, KP's to strike; "Where this goes", the remembering strings, the hub's dailies line, `BadgeDetail`'s refusal, the badge routes' weather, the three forms and the blend — KP's; `Navigation.tsx` carries KP's own uncommitted change (Playground → Library on the bar); a lamp never creates a test vessel in the live base until KP says.
