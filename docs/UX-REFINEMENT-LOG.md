...
- **Scope:** the split lifted into `src/lib/economics/split.ts` (+ `ledger.ts`, `recurrence.ts`, the tests) · six route renames with redirects · ② works beside wares, KP's stall, the five rungs · ④ the Loom as one form, two doors, the owner's shelf · ⑧ `/bazaar/works/[id]` · ⑥ recurrence at checkout, the webhook's ledger rows, the delivery road · ①③⑤⑦ to the canvas · `docs/sql/023-the-bazaar-refined-DRAFT.sql`.
