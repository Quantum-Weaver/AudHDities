...
A flex component is a **powerful layout system** that provides complete control over flexbox behavior. While Stack is opinionated (vertical or horizontal with fixed spacing), Flex is the **unopinionated foundation**—it gives you every flex property while maintaining our design token consistency.

**What it provides beyond Stack:**
- Full control over `flex-direction` (row, column, row-reverse, column-reverse)
- Wrap, wrap-reverse, and nowrap options
- Individual grow, shrink, and basis controls
- Gap support (using our spacing tokens)
- Alignment on both axes independently
- Responsive flex behavior

**When to use Stack vs Flex:**
- **Stack**: Simple vertical/horizontal layouts with consistent spacing
- **Flex**: Complex layouts requiring wrap, reverse, or custom growth behavior
...
| xs | 4px | `gap-1` | Very tight |
| sm | 8px | `gap-2` | Tight spacing |
| md | 16px | `gap-4` | Default |
| lg | 24px | `gap-6` | Comfortable |
| xl | 32px | `gap-8` | Generous |
| 2xl | 48px | `gap-12` | Wide |
| 3xl | 64px | `gap-16` | Very wide |
| 4xl | 96px | `gap-24` | Extreme |

---

## 🎯 **STACK VS FLEX: WHEN TO USE WHICH**
...
| Different grow/shrink ratios | ❌ | ✅ |
| Reverse order | ❌ | ✅ |
| Complex responsive direction | ❌ | ✅ |
| Custom align self per item | ❌ | ✅ |
| Minimal API | ✅ | ❌ |
