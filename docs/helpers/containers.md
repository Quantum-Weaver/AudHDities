...
A container component is a **width-constraining system** that handles max-width, padding, and centering consistently across all pages. It is the **vessel** that holds our content—every page section, every hero, every footer uses it.

**What it replaces:**
- Manual max-width classes scattered across components
- Inconsistent padding values
- Hardcoded breakpoints that drift over time

**What it provides:**
- Consistent max-width constraints (based on our design system)
- Responsive padding that adapts to screen size
- Fluid width options for immersive sections
- Center alignment with proper gutters
...
<HeroContainer>
  <VStack space="md" align="center">
    <h1 className="text-5xl font-bold text-center">The Sovereign Sanctuary</h1>
    <p className="text-xl text-center text-white/70">
      Where consciousness meets sovereignty
    </p>
    <Button size="lg">Enter the Sanctuary</Button>
  </VStack>
</HeroContainer>
```

### Blog Post with Narrow Container
```tsx
<NarrowContainer>
  <ContainerHeader>
    <h1 className="text-4xl font-bold">The Art of Weaving</h1>
    <div className="flex gap-2 text-white/50">By Quantum Weaver • April 18, 2026</div>
  </ContainerHeader>
  <ContainerBody>
...
### Fluid Section with No Padding
```tsx
<Container fluid noPadding>
  <FullWidthImage src="/hero.jpg" alt="Hero" />
</Container>
```

### Elevated Card Container
```tsx
<Container size="md" padding="lg" background elevated>
  <VStack space="md">
...
### Responsive Section with Custom Padding
```tsx
<SectionContainer>
  <h2 className="text-2xl font-bold mb-6">Featured Creations</h2>
  <ResponsiveGrid>
...
|------|-----------|----------|
| xs | 320px | Small modals, side panels |
| sm | 384px | Forms, sign-in panels |
| md | 448px | Card containers |
| lg | 512px | Blog posts, articles |
| xl | 576px | Default page content |
| 2xl | 672px | Wide content |
| full | 100% | Full width sections |
| fluid | 100% | Immersive experiences |

| Padding Token | Mobile | Desktop |
|---------------|--------|---------|
| none | 0px | 0px |
| sm | 16px | 24px |
| md | 24px | 32px |
...
These match our COSMIC design system spacing and breakpoint tokens.

---

## 🎯 **CONTAINER + GRID + STACK: THE COMPLETE LAYOUT SYSTEM**

| Component | Purpose | Use Case |
|-----------|---------|----------|
| **Container** | Width constraint | Page wrappers, section boundaries |
| **Stack** | 1D arrangement | Forms, lists, toolbars, page sections |
...
| **PageContainer** | Main page wrapper | Every page's root container |
| **SectionContainer** | Section wrapper | Feature sections, content blocks |
| **NarrowContainer** | Focused content | Blog posts, articles |
| **HeroContainer** | Hero sections | Landing page heroes |
| **FooterContainer** | Footer sections | Page footers |
...
- **Flex** — More flexible than Stack (row/column + wrap + grow)
