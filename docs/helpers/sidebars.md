...
A sidebar component is a **persistent navigation system** that provides access to primary sections of an application. It is the **spine** of the interface—every main section, every key action, every important destination lives in the sidebar.
...
<Sidebar
  items={[
    ...
    { id: 'orders', label: 'Orders', icon: <ShoppingCart />, href: '/orders', badge: 3 },
    { id: 'settings', label: 'Settings', icon: <Settings />, href: '/settings' },
  ]}
  header={<SidebarHeader brand="Sanctuary" />}
  showToggle
/>
```
  ...
  items={[
    { id: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard />, href: '/' },
    {
      ...
      children: [
        ...
        { id: 'add', label: 'Add New', href: '/products/new' },
      ],
    },
    { id: 'settings', label: 'Settings', icon: <Settings />, href: '/settings' },
  ]}
...
<Sidebar>
  <SidebarGroup label="Main">
    <SidebarNavItem item={{ id: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard />, href: '/' }} />
    <SidebarNavItem item={{ id: 'analytics', label: 'Analytics', icon: <TrendingUp />, href: '/analytics' }} />
  </SidebarGroup>
  <SidebarGroup label="Management">
    <SidebarNavItem item={{ id: 'users', label: 'Users', icon: <Users />, href: '/users' }} />
    <SidebarNavItem item={{ id: 'roles', label: 'Roles', icon: <Shield />, href: '/roles' }} />
  </SidebarGroup>
</Sidebar>
```

### Glass Variant Sidebar
```tsx
<Sidebar
  variant="glass"
  items={items}
  header={<SidebarHeader logo={<Logo />} brand="Sanctuary" />}
  footer={<SidebarFooter avatar={<Avatar src="/user.jpg" />} name="Quantum Weaver" email="weaver@sanctuary.com" />}
  showToggle
/>
```
...
### Using Sidebar Context (Custom Content)
```tsx
<Sidebar items={items} showToggle>
  <div className="mt-4 space-y-2">
    <div className="text-xs text-white/40 px-3">Recent Items</div>
    {/* Custom content here */}
  </div>
</Sidebar>
```
...
function CustomSidebarContent() {
  ...
  return (
    <div className="space-y-2">
      {!collapsed && <p className="text-sm">Additional content</p>}
      <Button size="sm" onClick={toggleCollapsed}>
...
|---------|------------|--------|----------|
| default | surface/95 | white/10 | Standard |
| glass | white/5 + blur | white/10 | Modern/immersive |
| minimal | transparent | white/5 | Subtle |

| Width | Expanded | Collapsed | Use Case |
|-------|----------|-----------|----------|
| default | 260px | 72px | Standard |
| custom | configurable | configurable | Custom layouts |

| Feature | Use Case |
|---------|----------|
| showToggle | User preference for collapsed state |
| mobileDrawer | Responsive/mobile layouts |
| nested items | Hierarchical navigation |
