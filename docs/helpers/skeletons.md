## ◻️ **SKELETON COMPONENT: Overview**

A skeleton component is a **loading placeholder system** that provides visual structure while content loads. It is the **ghost** of content yet to arrive—every loading state, every async component, every data-fetching boundary needs it.

**What it provides:**
- Pulse and wave animations for loading states
- Variants for different content types (text, avatar, image, card)
- Responsive sizing matching our design tokens
...
```tsx
<CardSkeletonComponent showImage textLines={3} />
```
...
```tsx
<Skeleton variant="card" animation="wave" className="w-full h-32" />
```
...
### Custom Dimensions
```tsx
<Skeleton variant="card" width={300} height={200} rounded />
```
...
| Size | Text Height | Avatar | Button | Use Case |
|------|-------------|--------|--------|----------|
| xs | 12px | 24x24 | 24x64 | Fine text |
| sm | 16px | 32x32 | 32x80 | Small text |
| md | 24px | 40x40 | 40x96 | Body text |
| lg | 32px | 48x48 | 48x112 | Subheadings |
| xl | 40px | 56x56 | 56x128 | Headings |
| 2xl | 48px | 64x64 | 64x160 | Large headings |
| 3xl | 64px | 96x96 | 80x192 | Hero text |
| 4xl | 96px | 128x128 | 96x224 | Display text |
