...
export function KnowledgeGallery() {
  ...
  useEffect(() => {
    fetch('/api/generated/mnemosyne-assessment/mythology?is_published=true&order=title.asc')
      .then(r => r.json()).then(result => { if (result.success) setItems(result.data?.data || result.data || []); })
      .catch(console.error).finally(() => setLoading(false));
  }, []);

  const filtered = useMemo(() => items.filter(i => i.title.toLowerCase().includes(searchTerm.toLowerCase()) || (i.content || '').toLowerCase().includes(searchTerm.toLowerCase())), [items, searchTerm]);
