...
export function CoursesGallery() {
  ...
  return (
    <main className="min-h-screen py-12"><div className="container max-w-6xl mx-auto px-6">
      ...
      {filtered.length === 0 && (<div className="text-center py-20"><GraduationCap className="h-12 w-12 text-star-dust/20 mx-auto mb-4" /><p className="text-star-dust/70 text-lg">{searchTerm ? 'No courses match' : 'The curriculum is being prepared'}</p></div>)}
        ...
        {filtered.map(c => {
          ...
          return (
            <Link key={c.id} href={`/library/courses/${c.slug}`}><Card data={cd} variant="interactive" radius="lg" shadow="sm" className="p-5 h-full">
              <div className="flex items-center justify-between mb-3">{c.difficulty && <Badge variant="outline" size="sm" className={`text-[10px] capitalize ${DIFFICULTY_COLORS[c.difficulty] || ''}`}>{c.difficulty}</Badge>}{c.estimated_duration && <span className="flex items-center gap-1 text-xs text-star-dust/70"><Clock size={12} />{c.estimated_duration}</span>}</div>
              <h3 className="text-lg font-semibold text-star-dust mb-2">{c.name}</h3><p className="text-sm text-star-dust/70 line-clamp-2">{c.description}</p>
