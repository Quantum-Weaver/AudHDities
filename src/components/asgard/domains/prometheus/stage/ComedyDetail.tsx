// src/components/asgard/domains/prometheus/stage/ComedyDetail.tsx
'use client';
...
export function ComedyDetail() {
  ...
  const [event, setEvent] = useState<ComedyEvent | null>(null);
  ...
  if (!event) return (<main className="min-h-screen py-12"><div className="container max-w-3xl mx-auto px-6 text-center"><Mic className="h-12 w-12 text-star-dust/20 mx-auto mb-4" /><p className="text-star-dust/40">This set has ended.</p><Link href="/stage/comedy" className="text-neurospark hover:underline mt-4 inline-block">Return to the Comedy Hearth</Link></div></main>);
  ...
  return (
    <main className="min-h-screen py-12"><div className="container max-w-3xl mx-auto px-6">
      <Link href="/stage/comedy" className="flex items-center gap-2 text-star-dust/60 hover:text-star-dust transition-colors text-sm mb-6"><ArrowLeft className="h-4 w-4" />Return to the Comedy Hearth</Link>
      <Card data={cd} variant="sanctuary" radius="xl" shadow="md" className="p-8">
        <div className="flex items-center gap-2 mb-4">{event.is_live && <Badge variant="outline" size="sm" className="text-[10px] bg-red-500/20 text-red-400 border-red-500/30">LIVE</Badge>}<Badge variant="outline" size="sm" className="text-[10px]">Comedy</Badge></div>
        ...
        <div className="flex items-center gap-4 text-sm text-star-dust/40 mb-6">
          ...
          <span className="flex items-center gap-1"><User size={14} />Comedian</span>
        </div>
