...
export function MusicGallery() {
  ...
  useEffect(() => {
    fetch('/api/generated/prometheus-stage/events?genre=music&order=scheduled_for.asc')
      .then(r => r.json())
  ...
  return (
    <main className="min-h-screen py-12"><div className="container max-w-6xl mx-auto px-6">
      <div className="mb-8"><Link href="/stage" className="flex items-center gap-2 text-star-dust/60 hover:text-star-dust transition-colors text-sm mb-2"><ArrowLeft className="h-4 w-4" />Return to the Stage</Link><h1 className="text-2xl font-bold text-star-dust">The Music Realm</h1><p className="text-sm text-star-dust/40 mt-1">Where sound becomes substance</p></div>
      {events.length === 0 ? (
        <div className="text-center py-20"><Music className="h-12 w-12 text-star-dust/20 mx-auto mb-4" /><p className="text-star-dust/40 text-lg">The realm awaits its first song</p></div>
      ) : (
        <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map(e => {
            const cd: CardData = { id: e.events_id, type: 'event', title: e.title, description: e.description || '' };
            return (
              <Link key={e.events_id} href={`/stage/music/${e.events_id}`}>
