...
import { Download, ImageIcon, Video, Music } from "lucide-react";
...
export function MediaAssets() {
  ...
  return (
    <div className="space-y-6">
        ...
        {mediaAssets.map((asset) => {
          ...
          return (
            <Card
              variant="glow"
              key={asset.id}
              data={{ id: asset.id, type: "value", title: asset.title, value: asset.format }}
              radius="lg"
              shadow="sm"
              className="overflow-hidden hover:border-neurospark/30 transition-all cursor-pointer"
              onClick={() => setSelectedAsset(asset)}
            >
              <div className="aspect-video w-full bg-deep-space/40 relative overflow-hidden">
                {asset.type === "video" ? (
                  <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-neurospark/10 to-quantum-purple/10">
                    <Video className="w-12 h-12 text-star-dust/20" />
                  </div>
                ) : (
                  <img
                ...
                <div className="absolute top-2 right-2">
                  <Badge variant="quantum" size="sm" className="bg-deep-space/50">
                    {typeLabels[asset.type]}
                  </Badge>
                </div>
              </div>
              <div className="p-4">
                <h3 className="font-semibold text-star-dust">{asset.title}</h3>
                <p className="text-sm text-star-dust/40 mt-1 line-clamp-2">
                  {asset.description}
                </p>
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center gap-2 text-xs text-star-dust/30">
      ...
      {selectedAsset && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-deep-space/80 backdrop-blur-sm"
        ...
        >
          ...
          >
            <div className="relative aspect-video bg-deep-space/40">
            ...
            <div className="p-6">
              <h3 className="text-xl font-bold text-star-dust">{selectedAsset.title}</h3>
              <p className="text-star-dust/60 mt-2">{selectedAsset.description}</p>
