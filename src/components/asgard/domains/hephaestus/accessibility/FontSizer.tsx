...
export function FontSizer() {
  ...
  return (
    <Card
      data={{ id: 'font-sizer', type: 'value', title: 'Font Size', value: '' }}
      radius="lg"
      shadow="md"
      variant="default"
      className="p-6"
    >
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" size="icon-sm" onClick={() => updateFontSize(fontSize - 1)} disabled={fontSize <= 12}>
            <Minus size={14} />
          </Button>
          <div className="flex-1">
            <Slider value={fontSize} onChange={handleSliderChange} min={12} max={24} step={1} />
          </div>
          <Button variant="outline" size="icon-sm" onClick={() => updateFontSize(fontSize + 1)} disabled={fontSize >= 24}>
            <Plus size={14} />
          </Button>
        </div>
