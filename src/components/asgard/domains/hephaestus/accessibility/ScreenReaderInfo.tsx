...
export function ScreenReaderInfo() {
  return (
    <Card
      data={{ id: 'screen-reader-info', type: 'value', title: 'Screen Reader Support', value: '' }}
      radius="lg"
      shadow="md"
      variant="default"
      className="p-6"
    >
      ...
      <div className="space-y-3">
        {screenReaders.map((sr) => (
          <div key={sr.name} className="flex items-center justify-between p-3 rounded-lg bg-white/5">
            <div>
              <span className="font-medium text-star-dust">{sr.name}</span>
              <span className="text-xs text-star-dust/40 ml-2">{sr.platform}</span>
            </div>
            <Badge variant={sr.support === 'full' ? 'success' : 'warning'} size="sm">
              {sr.support === 'full' ? 'Full Support' : 'Partial Support'}
            </Badge>
          </div>
        ))}
      </div>
      <div className="mt-4 p-3 bg-neurospark/10 border border-neurospark/20 rounded-lg">
