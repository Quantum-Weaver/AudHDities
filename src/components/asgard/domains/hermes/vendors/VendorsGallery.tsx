...
interface VendorItem {
  vendor_profiles_id: string; business_name: string; 
  ...
  return (
    <main className="min-h-screen py-12">
      <div className="container max-w-6xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((v) => (
            <Link key={v.vendor_profiles_id} href={`/bazaar/vendors/${v.vendor_profiles_id}`}>
              <VendorCardRenderer
                variant="interactive"
                radius="lg"
                shadow="sm"
                data={{
                  id: v.vendor_profiles_id, type: 'vendor', title: v.business_name,
