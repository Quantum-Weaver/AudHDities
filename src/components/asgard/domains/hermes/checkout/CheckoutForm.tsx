...
// ╔═══════════════════════════════════════════════════════════════════════════╗
// ║                    CHECKOUT FORM                                          ║
// ║                    Payment verification — zero hardcoded values           ║
// ╚═══════════════════════════════════════════════════════════════════════════╝
...
export function CheckoutForm({ sessionId, saleId, onSuccess, onError }: CheckoutFormProps) {
  ...
  if (status === "pending") {
    return (
      <Card data={{ id: 'checkout-process', type: 'value', title: 'Payment Processing', value: '' }} variant="default" radius="lg" shadow="md" className="p-8 text-center">
        ...
        <h2 className="text-xl font-semibold text-star-dust mb-2">Payment Processing</h2>
        <p className="text-star-dust/60 mb-4">Your payment is being processed. This may take a moment.</p>
  ...
  if (status === "error") {
    return (
      <Card data={{ id: 'checkout-error', type: 'value', title: 'Payment Failed', value: '' }} variant="default" radius="lg" shadow="md" className="p-8 text-center">
        <div className="w-12 h-12 rounded-full bg-error/20 flex items-center justify-center mx-auto mb-4">
          <AlertCircle className="w-6 h-6 text-error" />
        </div>
        <h2 className="text-xl font-semibold text-star-dust mb-2">Payment Failed</h2>
        <p className="text-star-dust/60 mb-4">{errorMessage || "Something went wrong. Please try again."}</p>
  ...
  return (
    <Card data={{ id: 'checkout-success', type: 'value', title: 'Payment Successful', value: '' }} variant="default" radius="lg" shadow="md" className="p-8 text-center">
