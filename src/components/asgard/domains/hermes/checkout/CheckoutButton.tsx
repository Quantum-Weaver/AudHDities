...
/** What the button needs of a ware or a work to offer the crossing. */
...
export function CheckoutButton({
  product, kind = 'ware', variant = 'primary', size = 'md', className = '', children
}: CheckoutButtonProps) {
  const { user } = useAuth();
  ...
  const inStock = product.quantity_available === null || product.quantity_available === undefined || product.quantity_available > 0;
  ...
  const handleCheckout = async () => {
    ...
    if (!user) {
      sessionStorage.setItem('pendingPurchase', JSON.stringify({ id: product.id, kind, quantity: 1 }));
      router.push(`/login?redirect=${encodeURIComponent(window.location.pathname)}`);
      return;
    }
    await initiateCheckout({ id: product.id, kind, quantity: 1 });
  };
