    ...
    <div className="border border-white/10 rounded-xl overflow-hidden">
      <div className="bg-white/5 px-4 py-3 border-b border-white/10">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-medium">Protected Routes</span>
        </div>
      </div>
      <div className="divide-y divide-white/5">
        {protectedRoutes.map((route) => (
          <div key={route.path} className="px-4 py-3 flex items-start gap-3">
            <Lock size={14} className="text-white/40 mt-1 flex-shrink-0" />
            <div>
              <code className="text-cyan-400 text-sm bg-cyan-400/10 px-2 py-0.5 rounded">
                {route.path}
              </code>
