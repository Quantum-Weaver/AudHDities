...
interface PrometheusErrorProps {
  error: Error & { digest?: string };
  reset: () => void;
}

export default function PrometheusError({ error, reset }: PrometheusErrorProps) {
  const router = useRouter();

  useEffect(() => {
    console.error('Stage & Loom error:', error);
  }, [error]);
    ...
    <main className="min-h-screen py-12">
      <div className="container max-w-3xl mx-auto px-6 text-center">
        ...
        <p className="text-star-dust/40 text-sm mb-6">
          Nothing you made is lost. The room can be asked for again.
        </p>
