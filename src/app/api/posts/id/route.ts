...
import { PostsInsertSchema } from 'src/lib/validators/posts';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('posts' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = PostsInsertSchema.parse(body);
    ...
    return errorResponse('Failed to create posts', 500);
  }
}
