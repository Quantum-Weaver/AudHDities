// =====================================================
// FILE: app/api/residual_pool/route.ts
// GENERATED: 2026-04-10T22:05:14.476Z
// =====================================================
...
import { ResidualPoolInsertSchema } from 'src/lib/validators/residual_pool';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('residual_pool' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = ResidualPoolInsertSchema.parse(body);
    
    const supabase = await createApiSupabase();
    const { data, error } = await supabase
      .from('residual_pool' as any)
    ...
    return errorResponse('Failed to create residual_pool', 500);
  }
}
