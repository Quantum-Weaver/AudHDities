...
import { PayoutsInsertSchema } from 'src/lib/validators/payouts';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('payouts' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    return errorResponse('Failed to create payouts', 500);
  }
}
