...
import { CommentsInsertSchema } from 'src/lib/validators/comments';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('comments' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = CommentsInsertSchema.parse(body);
    ...
    return errorResponse('Failed to create comments', 500);
  }
}
