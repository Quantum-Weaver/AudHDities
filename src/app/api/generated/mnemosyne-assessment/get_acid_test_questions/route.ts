...
export async function POST(request: NextRequest) {
  try {
    ...
    let { data, error } = await supabase.rpc('get_acid_test_questions', {
      ...body,
      p_user_id: userId
    });
    if (error && (error.code === 'PGRST202' || (error.message || '').includes('p_user_id'))) {
      ({ data, error } = await supabase.rpc('get_acid_test_questions', body));
    }

    if (error) throw error;
    return successResponse(data);
  } catch (error) {
    console.error('Error invoking get_acid_test_questions:', error);
    return errorResponse('Failed to invoke get_acid_test_questions', 500);
  }
}