...
import type { UserBadgesInsertSchema } from '@/lib/validators/generated/athena-gamification/user_badges.ts';
import { NextRequest } from 'next/server';

// =====================================================
// API ROUTE: /api/generated/athena-gamification/user_badges
// METHODS: GET, POST
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('user_badges').select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = UserBadgesInsertSchema.parse(body);
    ...
    console.error('Error creating user_badges:', error);
    return errorResponse('Failed to create user_badges', 500);
  }
}
