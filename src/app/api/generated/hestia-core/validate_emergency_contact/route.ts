...
export async function POST(request: NextRequest) {
  try {
    ...
    let { data, error } = await supabase.rpc('validate_emergency_contact', {
      ...body,
      p_user_id: userId
    });
    if (error && (error.code === 'PGRST202' || (error.message || '').includes('p_user_id'))) {
      ({ data, error } = await supabase.rpc('validate_emergency_contact', body));
    }

    if (error) throw error;
    return successResponse(data);
  } catch (error) {
    console.error('Error invoking validate_emergency_contact:', error);
    return errorResponse('Failed to invoke validate_emergency_contact', 500);
  }
}