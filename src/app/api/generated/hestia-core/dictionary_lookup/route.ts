...
export async function POST(request: NextRequest) {
  try {
    ...
    let { data, error } = await supabase.rpc('dictionary_lookup', {
      ...body,
      p_user_id: userId
    });
    if (error && (error.code === 'PGRST202' || (error.message || '').includes('p_user_id'))) {
      ({ data, error } = await supabase.rpc('dictionary_lookup', body));
    }

    if (error) throw error;
    return successResponse(data);
  } catch (error) {
    console.error('Error invoking dictionary_lookup:', error);
    return errorResponse('Failed to invoke dictionary_lookup', 500);
  }
}