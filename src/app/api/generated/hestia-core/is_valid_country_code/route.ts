...
export async function POST(request: NextRequest) {
  try {
    ...
    let { data, error } = await supabase.rpc('is_valid_country_code', {
      ...body,
      p_user_id: userId
    });
    if (error && (error.code === 'PGRST202' || (error.message || '').includes('p_user_id'))) {
      ({ data, error } = await supabase.rpc('is_valid_country_code', body));
    }

    if (error) throw error;
    return successResponse(data);
  } catch (error) {
    console.error('Error invoking is_valid_country_code:', error);
    return errorResponse('Failed to invoke is_valid_country_code', 500);
  }
}