...
import { WareParticipantsUpdateSchema } from '@/lib/validators/generated/hestia-core/ware_participants';
import { NextRequest } from 'next/server';

// Generated: 2026-07-10T18:14:59.979Z
...
export async function PUT(
  ...
  try {
    ...
    const validated = WareParticipantsUpdateSchema.parse(body);
    ...
    console.error('Error updating ware_participants:', error);
    return errorResponse('Failed to update ware_participants', 500);
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const supabase = await createApiSupabase();
    const { error } = await supabase.from('ware_participants').delete().eq('ware_participants_id', id);
