...
export async function POST(request: NextRequest) {
  try {
    ...
    let { data, error } = await supabase.rpc('gaia_sync', {
      ...body,
      p_user_id: userId
    });
    if (error && (error.code === 'PGRST202' || (error.message || '').includes('p_user_id'))) {
      ({ data, error } = await supabase.rpc('gaia_sync', body));
    }

    if (error) throw error;
    return successResponse(data);
  } catch (error) {
    console.error('Error invoking gaia_sync:', error);
    return errorResponse('Failed to invoke gaia_sync', 500);
  }
}