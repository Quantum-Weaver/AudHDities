...
export async function POST(request: NextRequest) {
  try {
    ...
    let { data, error } = await supabase.rpc('build_search_text', {
      ...body,
      p_user_id: userId
    });
    if (error && (error.code === 'PGRST202' || (error.message || '').includes('p_user_id'))) {
      ({ data, error } = await supabase.rpc('build_search_text', body));
    }

    if (error) throw error;
    return successResponse(data);
  } catch (error) {
    console.error('Error invoking build_search_text:', error);
    return errorResponse('Failed to invoke build_search_text', 500);
  }
}