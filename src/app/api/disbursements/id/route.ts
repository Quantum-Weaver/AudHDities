...
import { DisbursementsInsertSchema } from 'src/lib/validators/disbursements';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('disbursements' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = DisbursementsInsertSchema.parse(body);
    ...
    return errorResponse('Failed to create disbursements', 500);
  }
}
