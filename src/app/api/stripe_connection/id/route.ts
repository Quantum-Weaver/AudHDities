// =====================================================
// FILE: app/api/stripe_connection/route.ts
// GENERATED: 2026-04-13T21:47:21.278Z
// =====================================================
...
import { StripeConnectionInsertSchema } from 'src/lib/validators/stripe_connection';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('stripe_connection' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = StripeConnectionInsertSchema.parse(body);
    
    const supabase = await createApiSupabase();
    const { data, error } = await supabase
      .from('stripe_connection' as any)
    ...
    return errorResponse('Failed to create stripe_connection', 500);
  }
}
