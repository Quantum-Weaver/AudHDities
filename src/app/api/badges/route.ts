...
import { BadgesInsertSchema } from 'src/lib/validators/badges';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('badges' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    return errorResponse('Failed to create badges', 500);
  }
}
