...
import { ScenesInsertSchema } from 'src/lib/validators/scenes';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('scenes' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = ScenesInsertSchema.parse(body);
    ...
    return errorResponse('Failed to create scenes', 500);
  }
}
