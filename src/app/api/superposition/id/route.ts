// =====================================================
// FILE: app/api/superposition/route.ts
...
import { SuperpositionInsertSchema } from 'src/lib/validators/superposition';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('superposition' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = SuperpositionInsertSchema.parse(body);
    
    const supabase = await createApiSupabase();
    const { data, error } = await supabase
      .from('superposition' as any)
    ...
    return errorResponse('Failed to create superposition', 500);
  }
}
