...
import { NotificationsInsertSchema } from 'src/lib/validators/notifications';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('notifications' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = NotificationsInsertSchema.parse(body);
    ...
    return errorResponse('Failed to create notifications', 500);
  }
}
