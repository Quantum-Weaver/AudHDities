...
import { MessagesInsertSchema } from 'src/lib/validators/messages';
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('messages' as any).select('*', { count: 'exact' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    return errorResponse('Failed to create messages', 500);
  }
}
