...
import { Shield, Award, Heart, Sparkles, Calendar, MapPin, Star } from 'lucide-react';
...
export default async function ProfilePage({
  ...
  const [profileResult, communityResult, creatorResult, vendorResult, badgesResult] = await Promise.all([
    ...
    supabase.from('user_badges').select('*, badge').eq('user_id', targetId)
  ]);
  ...
  const tierInfo = {
    community: { color: 'text-green-400', bg: 'bg-green-500/10', border: 'border-green-500/20', icon: Heart, label: 'Community Member', description: 'Subsidized access for neurodivergent members' },
    ally: { color: 'text-cyan-400', bg: 'bg-cyan-500/10', border: 'border-cyan-500/20', icon: Shield, label: 'Ally', description: 'Supporters of the sanctuary' },
    corporate: { color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/20', icon: Award, label: 'Corporate Partner', description: 'Organizations aligned with our values' },
    council: { color: 'text-pink-400', bg: 'bg-pink-500/10', border: 'border-pink-500/20', icon: Sparkles, label: 'Council Member', description: 'Sovereign consciousness' },
  };
          ...
          <Card className="mb-8 p-6 bg-gradient-to-r from-white/5 to-transparent border border-white/10 overflow-hidden relative">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-6">
                {profile.acid_test_score !== null && (
                  <div className="text-center">
                    <div className="text-2xl font-bold text-white">{profile.acid_test_score}</div>
                    <div className="text-xs text-white/40">Acid Score</div>
                  </div>
                )}
                
                {profile.acid_test_persona && (
                  <div className="text-center">
                    <div className="text-sm text-cyan-400 capitalize">{profile.acid_test_persona.replace(/_/g, ' ')}</div>
                    <div className="text-xs text-white/40">Persona</div>
                  </div>
                )}
                
                {isOwnProfile && profile.user_tier !== 'council' && (
                  <a
                    href="/questionaire"
