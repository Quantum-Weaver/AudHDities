/* src/scripts/modules/format/format_validators.ts */
// Phase: format Zod validation schemas from table Row/Insert types
...
/**
 * format Zod schema for a table's Row type
 */
function formatRowSchema(tableName: string, rowContent: string): string {
...
/**
 * format Zod schema for a table's Insert type
 */
function formatInsertSchema(tableName: string, insertContent: string): string {
...
/**
 * format complete validator file content for a single table
 */
export function formatValidatorContent(tableName: string, rowContent: string, insertContent: string): string {
  const pascalName = toPascalCase(tableName);
  
  let content = `// =====================================================\n`;
  content += `// FILE: validators/ganerated/${tableName}.ts\n`;
  content += `// formatD FROM: database.types.ts\n`;
  content += `// =====================================================\n\n`;
  ...
  if (rowContent && rowContent.trim()) {
    content += formatRowSchema(tableName, rowContent) + '\n\n';
  }
  
  if (insertContent && insertContent.trim()) {
    content += formatInsertSchema(tableName, insertContent) + '\n\n';
  }
...
/**
 * format validator file for a single table
 */
export async function formatValidatorForTable(
  tableName: string,
  tableContent: string,
  ...
  const content = formatValidatorContent(tableName, rowContent, insertContent);
...
/**
 * format validators for multiple tables
 */
export async function formatValidatorsForTables(
  ...
  for (const table of tables) {
    const fileResult = await formatValidatorForTable(table.name, table.content, options);
