...
function main(): void {
  console.log('\n');
  logSeparator();
  logStep('STEP 5: Constants Section Isolation');
  logSeparator();
  console.log('\n');
  ...
  const constantsSection = extractSection(content, marker);
  
  if (constantsSection) {
    logSuccess(`Extracted Constants section`);
    logInfo(`Length: ${constantsSection.length} characters`);
    
    // Show preview
    const preview = constantsSection.substring(0, 200).replace(/\n/g, '\\n');
    logInfo(`Preview: ${preview}...`);
    
    // Optional: Save to file for inspection
    // fs.writeFileSync('constants-section.txt', constantsSection);
  } else {
