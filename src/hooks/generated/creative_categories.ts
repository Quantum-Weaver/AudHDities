...
export function useCreativeCategories(id: string | undefined) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/generated/creative_categories/${id}`);
...
export function useCreativeCategoriesList(params?: {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `/api/generated/creative_categories?${searchParams.toString()}`;
...
export function useCreateCreativeCategories() {
  ...
  const create = useCallback(async (data: CreativeCategoriesInsert) => {
    ...
    try {
      const response = await fetch('/api/generated/creative_categories', {
        method: 'POST',
