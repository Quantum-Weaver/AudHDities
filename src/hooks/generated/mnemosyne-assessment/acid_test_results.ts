// src/hooks/generated/mnemosyne-assessment/acid_test_results.ts
// =====================================================
// HOOKS: acid_test_results
...
export function useAcidTestResults(id: string | undefined) {
  const [data, setData] = useState<AcidTestResultsRow | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/generated/mnemosyne-assessment/acid_test_results/${id}`);
...
export function useAcidTestResultsList(params?: {
  page?: number; limit?: number; filters?: Record<string, string>; sort?: string; order?: 'asc' | 'desc';
}) {
  const [data, setData] = useState<AcidTestResultsRow[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/generated/mnemosyne-assessment/acid_test_results?${searchParams.toString()}`);
      const result = await response.json();
      if (result.success) {
...
export function useCreateAcidTestResults() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const create = useCallback(async (data: AcidTestResultsInsert) => {
...
export function useUpdateAcidTestResults() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const update = useCallback(async (id: string, data: AcidTestResultsUpdate) => {
    setLoading(true); setError(null);
    try {
      const response = await fetch(`/api/generated/mnemosyne-assessment/acid_test_results/${id}`, {
...
export function useDeleteAcidTestResults() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const deleteRecord = useCallback(async (id: string) => {
    setLoading(true); setError(null);
    try {
      const response = await fetch(`/api/generated/mnemosyne-assessment/acid_test_results/${id}`, { method: 'DELETE' });
      const result = await response.json();
