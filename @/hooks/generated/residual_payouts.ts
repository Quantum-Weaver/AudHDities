...
export function useResidualPayouts(id: string | undefined) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/generated/residual_payouts/${id}`);
...
export function useResidualPayoutsList(params?: {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `/api/generated/residual_payouts?${searchParams.toString()}`;
...
export function useCreateResidualPayouts() {
  ...
  const create = useCallback(async (data: ResidualPayoutsInsert) => {
    ...
    try {
      const response = await fetch('/api/generated/residual_payouts', {
        method: 'POST',
