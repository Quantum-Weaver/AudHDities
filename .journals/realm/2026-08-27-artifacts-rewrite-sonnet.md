...
## Addendum — the map, one dialog

LearscailScroll's roll-out row and Navigation's dead `hidden md:flex` overlay (no trigger ever set `mapOpen` true) both retired into one native `<dialog>` (new `MapDialog.tsx`, mounted once in `LayoutChrome`, opened via `ContinuityBeamContext`'s new `mapOpen`); measured before: panel `left:0 width:1024` on a 1425px viewport (not centered — traced to `globals.css:21`'s unlayered `* { margin/padding: 0 }`, which beats every Tailwind `m-*`/`p-*` utility app-wide by cascade-layer rule, confirmed via computed style, worked around here with inline `style` + `gap-*` inside `MapDialog.tsx` only); measured after: `dialog left:136.5 right:1288.5 width:1152` on the same 1425px viewport, exactly `(1425−1152)/2`, and `top:90` = `(900−720)/2`, both centered on the nose; Esc, backdrop-click, and the close button each verified closing it (`dialog.open → false`), a click inside the panel verified not closing it, and the same 80vw×80vh centering held at a 390×844 mobile viewport. `npx tsc --noEmit`: 0 errors. `npx next build`: clean, both before rebuild (for the honest before-measurement, via `git stash` of the four edited files) and after.
